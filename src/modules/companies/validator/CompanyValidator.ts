...
const companyValidator = {
  [Segments.BODY]: Joi.object().keys({
    ...
    whatsapp: Joi.string().min(11).empty("").messages(customMessage("whatsapp")),
    website: Joi.string().uri().empty("").messages(customMessage("website")),
    cep: Joi.string().length(9).messages(customMessage("cep")),
